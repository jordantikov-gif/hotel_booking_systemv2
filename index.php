...
// Статичен масив с максимален капацитет за всеки ден
$capacityPerDay = [
    1 => 5, 2 => 8, 3 => 1, 4 => 6, 5 => 7, 6 => 4,
    7 => 9, 8 => 2, 9 => 10, 10 => 5, 11 => 7, 12 => 3,
    13 => 6, 14 => 8, 15 => 1, 16 => 9, 17 => 4, 18 => 10,
    19 => 7, 20 => 5, 21 => 6, 22 => 3, 23 => 8, 24 => 4,
    25 => 10, 26 => 6, 27 => 5, 28 => 7, 29 => 0, 30 => 9, 31 => 8
];

$hotel = "Хотел A"; // текущият хотел
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($guests < 1 || $guests > 10) {
        ...
        $datesArray = explode(',', $dates); // Превръщаме избраните дати в масив
        $successCount = 0;

        foreach($datesArray as $d){
            // Подготовка на заявка за вмъкване на резервация
            $stmt = $conn->prepare(
                "INSERT INTO reservations 
                (hotel, name, email, phone, guests, date, time, notes, price) 
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
            );

            // Свързваме променливите с подготвената заявка
            $stmt->bind_param(
                "sssissssd", 
                $hotel, $name, $email, $phone, $guests, $d, $time, $notes, $price
            );
...
// Изчисляваме реалния капацитет само за текущия хотел
$currentCapacity = $capacityPerDay;

// Тук използваме подготвена заявка с параметър hotel, за да вземем резервациите само за този хотел
$stmt = $conn->prepare("
...
if($result){
    while($row = $result->fetch_assoc()){
        ...
        if(isset($currentCapacity[$day])){
            // Актуализираме текущия капацитет, като изваждаме вече заетите места
            $currentCapacity[$day] -= $booked;
            if($currentCapacity[$day] < 0) $currentCapacity[$day] = 0; // не може да има отрицателни места
        }
    }
}
$stmt->close();
...
<head>
<meta charset="UTF-8">
<title>Резервация - Хотел A</title>
...
<h2 class="mb-3 text-center">Направи резервация - Хотел A</h2>
...
const monthNames = ["Януари","Февруари","Март","Април","Май","Юни","Юли","Август","Септември","Октомври","Ноември"];

// Взимаме капацитета от PHP масива, преобразуван в JSON
const currentCapacity = <?php echo json_encode($currentCapacity); ?>;

// Функция за водене на водеща нула при форматиране на датата
function pad(n){ return n.toString().padStart(2,'0'); }

// Форматиране на дата в "YYYY-MM-DD" за изпращане към PHP
function formatDate(d, m, y){ return y + "-" + pad(m+1) + "-" + pad(d); }

// Пресмятане на общата цена
...
function renderCalendar(year, month) {
    ...
    for (let d=1; d<=daysInMonth; d++) {
        ...
        let free = currentCapacity[d] || 0; // свободни места за деня
        ...
        if(free <=0){
            ...
            // Денят има свободни места
            div.innerHTML = d + "<span>"+free+" места</span>";
            div.addEventListener("click", ()=> {
                const guests = parseInt(guestsInput.value) || 0;

                if(guests <= 0) { 
                    alert("Моля, въведете брой гости"); 
                    return; 
                }

                if(guests > free){ 
                    alert("Няма достатъчно свободни места за този ден"); 
                    return; 
                }

                // Актуализиране на капацитета локално в JS
                ...
                // Записване на избраните дати в скритото поле за POST
...
// Спираме бутоните за навигация между месеци
